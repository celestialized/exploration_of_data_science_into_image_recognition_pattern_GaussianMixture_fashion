...
def train_image_features_extracor(directory):
    train_X = list()
    #fashion_train_file = open("data/fashion_train/fashion_train.txt", "a")
    for dossier in os.listdir(directory):
        if(dossier.title()=="0"):
            ...
            for img in os.listdir(directory + "/9/"):
                ...
                image = img_to_array(image)
                train_X.append(image)
                #fashion_train_file.write("9\n")
    save_files(data_generated_path, 'train_images_features.pkl', train_X)
    dump(train_X, open('data_generated/featuresTrainX.pkl', 'wb'))
    return train_X

def eval_image_features_extractor(directory):
    test_X = list()
    #fashion_test_file = open("data/fashion_test/fashion_test.txt", "a")
    for dossier in os.listdir(directory):
        if(dossier.title()=="0"):
            for img in os.listdir(directory + "/0/"):
                # print img
            ...
            for img in os.listdir(directory + "/9/"):
                ...
                image = img_to_array(image)
                test_X.append(image)
                #fashion_test_file.write("9\n")
    save_files(data_generated_path, 'test_images_features.pkl', test_X)
    dump(test_X, open('data_generated/featuresTestX.pkl', 'wb'))
    return test_X
...
def imageLoader():
    ...
    dump(trainX, open('data_generated/featuresTrainX.pkl', 'wb'))
    dump(testX, open('data_generated/featuresTestX.pkl', 'wb'))
